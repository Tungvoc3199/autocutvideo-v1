...
- Include resolved path, file size, content SHA-256, and `mtime_ns` in metadata; use the stable content fields for job identity so timestamp-only changes remain resumable while replaced videos create new jobs.
- Keep `resume --job` in the explicitly requested output folder and reject resume when the source video's content changed.
