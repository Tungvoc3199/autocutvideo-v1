...
- Job id được tính từ path đã resolve, kích thước file, và SHA-256 nội dung video; `mtime_ns` vẫn được lưu trong metadata để chẩn đoán. Nếu chỉ timestamp file thay đổi, job cũ vẫn resume đúng folder. Nếu bạn thay video mới vào cùng một path, tool tạo job mới thay vì dùng lại subtitle/audio cũ.
...
- Resume luôn tiếp tục đúng folder được truyền qua `--job`, chạy lại stage chưa hoàn tất hoặc stage failed, và từ chối resume nếu nội dung video gốc đã thay đổi. Dùng `--force` để chạy lại toàn bộ stage đã completed của cùng video.
...
- Manual resume
- Manual resume sau khi timestamp file nguồn thay đổi
